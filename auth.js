...
router.post('/login_process', function (request, response) {
    ...
    if (username && password) {             // id와 pw가 입력되었는지 확인
        ...
        db.query('SELECT * FROM board WHERE username = ? AND password = ?', [username, password], function(error, results, fields) {
...
router.post('/register_process', function(request, response) {    
    ...
    if (username && password && password2) {
        ...
        db.query('SELECT * FROM board WHERE username = ?', [username], function(error, results, fields) { // DB에 같은 이름의 회원아이디가 있는지 확인
            if (error) throw error;
            if (results.length <= 0 && password == password2) {     // DB에 같은 이름의 회원아이디가 없고, 비밀번호가 올바르게 입력된 경우 
                db.query('INSERT INTO board (username, password) VALUES(?,?)', [username, password], function (error, data) {
